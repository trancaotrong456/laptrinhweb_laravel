...
<body>
    ...
    <nav class="navbar navbar-expand-lg">
        ...
        <div class="container-fluid">
            ...
            <div class="collapse navbar-collapse" id="navbarContent">
                ...
                <!-- SEARCH -->
                <form class="search-box me-lg-4" id="navSearchForm">
                    <input type="text" class="search-input" id="navSearchInput" placeholder="Tìm kiếm sản phẩm, danh mục...">
                    <i class="fas fa-search search-icon" id="navSearchBtn"></i>
                    <div id="navSearchSuggestions" class="search-suggestions d-none"></div>
                </form>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        searchInput.addEventListener('input', function() {
            ...
            searchTimeout = setTimeout(function() {
                ...
                fetch(`{{ route('products.search') }}?q=${encodeURIComponent(query)}`)
                    ...
                    .then(products => {
                        // Fetch categories
                        return fetch(`{{ route('categories.searchSuggestions') }}?q=${encodeURIComponent(query)}`)
                            .then(response => response.json())
                            .then(categories => ({ products, categories }));
                    })
                    .then(data => {
                        searchSuggestions.innerHTML = '';
                        ...
                        if (products.length > 0) {
                            ...
                            productsHeader.className = 'px-3 py-2 text-muted fw-bold small border-bottom';
                            productsHeader.innerHTML = '<i class="fas fa-box me-2"></i>Sản phẩm';
                            searchSuggestions.appendChild(productsHeader);

                            products.forEach(product => {
                                const a = document.createElement('a');
                                a.href = `{{ route('products.detail', ['product' => 'ID']) }}`.replace('ID', product.id);
                                ...
                                a.innerHTML = `
                                    ...
                                    <span>
                                        <span class="search-suggestion-title d-block">${escapeHtml(product.name)}</span>
                                        <span class="search-suggestion-desc d-block">${product.price ? new Intl.NumberFormat('vi-VN').format(product.price) + ' đ' : ''}</span>
                                    </span>
                                `;
                        ...
                        if (categories.length > 0) {
                            ...
                            categoriesHeader.className = 'px-3 py-2 text-muted fw-bold small border-top border-bottom';
                            categoriesHeader.innerHTML = '<i class="fas fa-layer-group me-2"></i>Danh mục';
                            searchSuggestions.appendChild(categoriesHeader);
                            ...
                            categories.forEach(category => {
                                ...
                                button.innerHTML = `
                                    ...
                                    <span>
                                        <span class="search-suggestion-title d-block">${escapeHtml(category.name)}</span>
                                        <span class="search-suggestion-desc d-block">${escapeHtml(category.description || '')}</span>
                                    </span>
                                `;

                                button.addEventListener('click', function() {
                                    searchInput.value = category.name;
                                    window.location.href = `{{ route('categories.index') }}?search=${encodeURIComponent(category.name)}`;
                                });
        ...
        searchBtn.addEventListener('click', function(e) {
            ...
            if (keyword) {
                window.location.href = `{{ route('products.index') }}?keyword=${encodeURIComponent(keyword)}`;
            }
        });
