...
class SavedCouponController extends Controller
{
    ...
    public function save(Request $request, $id)
    {
        ...
        if (!$coupon) {
            if ($request->ajax() || $request->wantsJson()) {
                return response()->json(['success' => false, 'message' => 'Mã giảm giá không tồn tại hoặc đã bị vô hiệu hóa.'], 422);
            }
            return back()->with('error', 'Mã giảm giá không tồn tại hoặc đã bị vô hiệu hóa.');
        }
        ...
        if ($exists) {
            ...
            return back()->with('error', 'Bạn đã lưu mã giảm giá này rồi.');
        }
    ...
    public function unsave(Request $request, $id)
    {
        ...
        if (!$savedCoupon) {
            ...
            return back()->with('error', 'Không tìm thấy mã giảm giá này trong ví của bạn.');
        }
