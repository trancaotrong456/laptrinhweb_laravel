...
class CartController extends Controller
{
    ...
    public function undoRemove()
    {
        ...
        if (!is_array($lastRemoved) || !isset($lastRemoved['product_id'], $lastRemoved['item'])) {
            return redirect()->route('cart.index')
                ->with('error', 'Khong co san pham nao de hoan tac.');
        }
        ...
        if ($item) {
            $item->quantity += (int) ($restoredItem['quantity'] ?? 1);
            $item->save();
        } else {
            UserCartItem::create([
                'user_id' => $userId,
                'product_id' => $productId,
                'product_name' => $restoredItem['name'] ?? 'San pham',
        ...
        return redirect()->route('cart.index')
            ->with('success', 'Da hoan tac san pham vua xoa.');
    }
    ...
    public function clear()
    {
        ...
        return redirect()->route('cart.index')
            ->with('success', 'Gio hang da trong!');
    }
    ...
    public function applyCoupon(Request $request)
    {
        ...
        if (!$coupon || !$coupon->isCurrentlyValid()) {
            return redirect()->route('cart.index')->with('error', 'Ma giam gia khong hop le hoac da het han.');
        }

        if (!is_null($coupon->min_order_value) && $subtotal < (float) $coupon->min_order_value) {
            return redirect()->route('cart.index')
                ->with('error', 'Don hang chua dat muc toi thieu de ap dung ma nay.');
        }
        ...
        return redirect()->route('cart.index')
            ->with('success', 'Ap dung ma giam gia thanh cong!');
    }
    ...
    public function removeCoupon()
    {
        ...
        return redirect()->route('cart.index')
            ->with('success', 'Da bo ma giam gia.');
    }
    ...
    public function checkout(Request $request)
    {
        ...
        if (empty($cart)) {
            return redirect()->route('cart.index')
                ->with('error', 'Gio hang trong, khong the thanh toan!');
        }

        $selectedIds = array_map('strval', $request->input('selected_items', []));
        $selectedCart = [];

        foreach ($selectedIds as $selectedId) {
            if (isset($cart[$selectedId])) {
                $selectedCart[$selectedId] = $cart[$selectedId];
            }
        }

        if (empty($selectedCart)) {
            return redirect()->route('cart.index')
                ->with('error', 'Vui long chon it nhat 1 san pham de thanh toan.');
        }

        $subtotal = $this->calculateSubtotal($selectedCart);
        ...
        if ($couponCode !== '') {
            $couponUsed = Coupon::whereRaw('UPPER(code) = ?', [$couponCode])->first();

            if (!$couponUsed || !$couponUsed->isCurrentlyValid()) {
                return redirect()->route('cart.index')
                    ->with('error', 'Ma giam gia khong hop le hoac da het han.');
            }

            if (!is_null($couponUsed->min_order_value) && $subtotal < (float) $couponUsed->min_order_value) {
                return redirect()->route('cart.index')
                    ->with('error', 'Don hang duoc chon chua dat muc toi thieu cua ma giam gia.');
            }

            $discount = $this->calculateCouponDiscount($couponUsed, $subtotal);
        }

        $shippingFee = (($subtotal - $discount) >= self::FREE_SHIPPING_THRESHOLD) ? 0 : self::SHIPPING_FEE;
        $grandTotal = max(0, $subtotal - $discount) + $shippingFee;

        DB::transaction(function () use ($selectedCart, $couponUsed) {
            $userId = auth()->id();

            foreach (array_keys($selectedCart) as $selectedProductId) {
        ...
        return redirect()->route('order.confirmation')
            ->with('success', 'Don hang duoc tao thanh cong!');
    }
    ...
    public function orderConfirmation()
    {
        ...
        if (!$order) {
            return redirect()->route('home')
                ->with('error', 'Khong tim thay don hang!');
        }
    ...
    {
        $code = (string) data_get(Session::get('cart_coupon', []), 'code', '');
        ...
        if (!$coupon || !$coupon->isCurrentlyValid()) {
            ...
            return [
                ...
                'discount' => 0,
                'meta' => null,
                'hint' => 'Ma giam gia khong con hop le.',
            ];
        }
        ...
        if ($minOrder > 0 && $subtotal < $minOrder) {
            return [
                ...
                'hint' => 'Don hang chua dat gia tri toi thieu de an ma.',
            ];
        }
