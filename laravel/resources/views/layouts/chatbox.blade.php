...
@php
    ...
    // Lấy thông tin đơn hàng của khách hàng để AI hỗ trợ
    ...
    foreach ($userOrders as $order) {
        $tongTien = number_format($order->total, 0, ',', '.');
        $ngayDat = $order->created_at->format('d/m/Y H:i');
        $trangThai = match($order->status) {
            'pending' => 'Chờ xử lý',
        ...
        $danhSachDonHang .= "- Mã đơn: {$order->order_number} | Trạng thái: {$trangThai} | Tổng: {$tongTien}đ | Ngày đặt: {$ngayDat}\n";
        $danhSachDonHang .= "  Sản phẩm mua: ";
        foreach($order->items as $item) {
             $tenSP = $item->product ? $item->product->name : 'Sản phẩm không xác định';
             $danhSachDonHang .= "{$tenSP} (x{$item->quantity}), ";
        }
        $danhSachDonHang = rtrim($danhSachDonHang, ", ") . "\n";
    }
    if (empty(trim($danhSachDonHang))) {
        $danhSachDonHang = 'Khách hàng hiện chưa có đơn hàng nào.';
    }
@endphp
...
(function() {
    ...
    const danhSachDonHang = {!! json_encode($danhSachDonHang) !!};
...
${danhSachDonHang}
Nếu khách hỏi về tình trạng đơn hàng, hãy xem danh sách trên để trả lời. Khách hỏi gì thì trả lời ngắn gọn, đúng trọng tâm thông tin đơn hàng đó.
