...
<style>
.product-detail-page {
    background: radial-gradient(circle at top left, rgba(249,115,22,.12), transparent 28%),
...
<div class="product-detail-page">
    <div class="container detail-shell">
            ...
            <div class="row g-4 align-items-stretch">
                ...
                <div class="col-lg-6">
                    <div class="h-100 d-flex flex-column p-lg-3">
                        @if($product->category)
                            <span class="badge rounded-pill text-bg-primary px-3 py-2 mb-3" style="width:fit-content">
                        ...
                        <div class="mt-auto">
                            ...
                            @else
                                <div class="d-grid d-sm-flex gap-2">
                                    ...
                                    <a href="#reviews" class="btn action-button btn-cart-action">Xem đánh giá</a>
                                </div>
                            @endauth
                        </div>
                    </div>
        ...
        <div class="detail-tabs mt-4" id="detailTabs">
            <div class="detail-tab active" data-tab="desc">Mô tả sản phẩm</div>
            <div class="detail-tab" data-tab="nutrition">Thông tin dinh dưỡng</div>
            <div class="detail-tab" data-tab="reviews">Đánh giá ({{ $reviewCount }})</div>
        </div>
        <div class="detail-tab-content active" id="tab-desc">
            <p>{{ $product->description ?: 'Sản phẩm chất lượng cao, được chọn lọc kỹ càng từ các vùng canh tác uy tín.' }}</p>
            <ul>
                <li>Sản phẩm tươi ngon, đảm bảo chất lượng</li>
                <li>Xuất xứ rõ ràng, truy xuất nguồn gốc đầy đủ</li>
                <li>Giao hàng nhanh, đóng gói cẩn thận</li>
            </ul>
        </div>
        <div class="detail-tab-content" id="tab-nutrition">
            <p>Thông tin dinh dưỡng sẽ được cập nhật sớm.</p>
        </div>
        <div class="detail-tab-content" id="tab-reviews">
            {{-- Review summary & form --}}
            <div class="row g-4 mb-5 pt-2" id="reviews">
                <div class="col-lg-5">
                    <div class="review-panel p-4 h-100">
                        ...
                        @for($rating = 5; $rating >= 1; $rating--)
                            ...
                            @endphp
                            <div class="d-flex align-items-center gap-2 mb-3">
                                <div class="text-nowrap" style="width:42px;">{{ $rating }} <i class="fas fa-star text-dark"></i></div>
                    ...
                    <div class="review-panel p-4 h-100">
                        ...
                        @else
                            <div class="p-4 rounded-4 bg-light text-center">
                                ...
                                <a href="{{ route('login') }}" class="btn btn-primary rounded-pill px-4">Đăng nhập</a>
                            </div>
                        @endauth
                    </div>
                </div>
                                ...
                                <div class="d-flex gap-3">
                                    ...
                                    <div class="flex-grow-1">
                                        <div class="d-flex flex-wrap align-items-center gap-2 mb-2">
                                            ...
                                            <span class="text-success small"><i class="fas fa-circle-check me-1"></i>Khách hàng đã đăng nhập</span>
...
<script>
    ...
    // Star rating highlight (for the review form in the right panel)
    function highlightStars(n) {
        document.querySelectorAll('[id^="rs"]').forEach(radio => {
            const lbl = document.querySelector('label[for="'+radio.id+'"]');
            if (!lbl) return;
            const val = parseInt(radio.value);
            lbl.style.color = val <= n ? '#f9a825' : '#e0e0e0';
        });
    }
    function resetStars() {
        ...
        document.querySelectorAll('[id^="rs"]').forEach(radio => {
            const lbl = document.querySelector('label[for="'+radio.id+'"]');
            if (!lbl) return;
            lbl.style.color = parseInt(radio.value) <= val ? '#f9a825' : '#e0e0e0';
        });
    }
    function selectStar(n) {
        document.querySelectorAll('[id^="rs"]').forEach(r => {
            if (parseInt(r.value) === n) r.checked = true;
        });
        resetStars();
    }
    // Initialize stars
